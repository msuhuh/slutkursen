...
for _, norm_row in data_with_norm.iterrows():
    virus_accession = norm_row["Accession Number"]
    virus_family = norm_row["virus_taxa_family"]
    human_accession = norm_row["human_accession"]

    # Check if the accession and virus family exist in the matrix
    if human_accession in matrix["human_accession"].values and virus_family in matrix.columns:
        #print("in if")
        ...
        normalized_data.loc[normalized_data["human_accession"] == human_accession, virus_family] /= (norm_length * norm_family)
    else:
        # Record unmatched entries
        unmatched_entries.append((human_accession, virus_family))
