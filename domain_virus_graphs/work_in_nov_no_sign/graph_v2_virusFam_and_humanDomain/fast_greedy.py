...
def test_clustering_algorithms(g_path):
    ...
    algorithms = {
        "Fast Greedy": lambda weights: g.community_fastgreedy(weights=weights).as_clustering(),
    }
    ...
    for algorithm_name, algorithm_fn in algorithms.items():
        ...
        for weights in [None, "weight"]:
            weight_label = "no_weights" if weights is None else "with_weights"
            print(f"  Testing with weights={weight_label}")

            try:
                communities = algorithm_fn(weights)
                if communities is not None:
                    output_dir = os.path.join(
                        "local_code/graph_v2_virusFam_and_humanDomain/results_brute_force",
                        "fast_greedy"
                    )
                    params = f"{network_name}_{weight_label}"
                    analyze_and_save_clusters(g, communities, algorithm_name, params, output_dir)
            except Exception as e:
                print(f"Error with {algorithm_name} and weights={weight_label}: {e}")
