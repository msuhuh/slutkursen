...
def test_clustering_algorithms(g_path):
    ...
    algorithms = {
        "Leading Eigenvector": lambda clusters: g.community_leading_eigenvector(
            clusters=clusters,
            weights= g.es["weight"]
        ),
    }
    ...
    for algorithm_name, algorithm_fn in algorithms.items():
        ...
        # Test with clusters=None and clusters from 2 to 10
        for clusters in [None] + list(range(2, 11)):
            cluster_label = "clusters_None" if clusters is None else f"clusters_{clusters}"
            print(f"  Testing with {cluster_label}")
            ...
            try:
                ...
                if communities is not None:
                    ...
                    params = f"{network_name}_{cluster_label}"
                    analyze_and_save_clusters(g, communities, algorithm_name, params, output_dir)
            except Exception as e:
                print(f"Error with {algorithm_name} and {cluster_label}: {e}")
